...
FUNCTION disp_contacts()
	DISPLAY ARRAY m_contList TO arr.* ATTRIBUTES(DOUBLECLICK=accept, CANCEL=FALSE) -- uncomment to it exiting! ATTRIBUTES(ACCEPT=FALSE,CANCEL=FALSE)
		BEFORE ROW
        ...
        ON ACTION markasdead ATTRIBUTES(ROWBOUND, TEXT="Mark as dead")
            MESSAGE "Has died"
        ON ACTION markasenemy ATTRIBUTES(ROWBOUND, TEXT="Mark as enemy")
            MESSAGE "Enemy now"
        ON ACTION markasrevived ATTRIBUTES(ROWBOUND, TEXT="Mark as revived")
            MESSAGE "It's a zombie"
        ON ACTION accept
            --CALL fgl_winmessage("Message","Accept fired for row " || arr_curr(), "info")
            MESSAGE "Accept fired for row " || arr_curr()
...
--	DISPLAY "Img:",l_img," - ",os.path.join(getImagePath(),l_img)
	IF NOT os.path.exists( os.path.join(getImagePath(),l_img) ) THEN
		LET l_img = os.path.join(C_IMGPATH,l_nam)||".png"
		IF NOT os.path.exists( os.path.join(getImagePath(),l_img) ) THEN
